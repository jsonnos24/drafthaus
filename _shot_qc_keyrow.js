...
await p.goto(`http://127.0.0.1:${port}/lite-1.037.html`,{waitUntil:'load'});
await p.click('.auth-btn.ghost'); await p.waitForSelector('body.signed-in',{timeout:15000});
await p.evaluate(()=>{_openSongObj({id:'S',title:'X',key:'C major',lyricsDoc:'<div>x</div>'});stopTakesListener();qcRender();_qcSelected='G';openQuickChords();qcPlayPill&&null;qcRender();});
await p.waitForTimeout(150);
await p.screenshot({path:'_shot_qc_keyrow.png'});
