// _verify_lite_1080.js — lite-1.080: never auto-use continuity (iPhone/iPad) mics
...
function serve() {
  return new Promise(res => {
    const s = http.createServer((req, rq) => {
      let p = decodeURIComponent(req.url.split('?')[0]); if (p === '/') p = '/lite-1.080.html';
...
(async () => {
  ...
  await page.goto(`http://localhost:${port}/lite-1.080.html`, { waitUntil: 'domcontentloaded' });
  ...
  ok(a3.gum.length === 2 && a3.gum[0].audio && a3.gum[0].audio.deviceId && a3.gum[0].audio.deviceId.ideal === 'FAKEDEV123'
     && a3.gum[0].audio.echoCancellation === false
     && a3.gum[1].audio && a3.gum[1].audio.deviceId && a3.gum[1].audio.deviceId.exact === 'FAKEDEV123',
    'A3a startRecord requests saved deviceId (ideal), then retries with exact on mismatch');
  ...
  // ── E2: never auto-use a continuity (iPhone/iPad) mic when nothing is saved ──
  const e2 = await page.evaluate(async () => {
    const origEnum = navigator.mediaDevices.enumerateDevices;
    navigator.mediaDevices.enumerateDevices = async () => ([
      { kind: 'audioinput', deviceId: 'default', label: "Default - Jason's iPhone Microphone (Continuity)" },
      { kind: 'audioinput', deviceId: 'IPH', label: "Jason's iPhone Microphone (Continuity)" },
      { kind: 'audioinput', deviceId: 'MAC1', label: 'MacBook Air Microphone (Built-in)' },
    ]);
    localStorage.removeItem('dh-lite-input-device');
    const noSaved = await recResolveAudioConstraints();
    localStorage.setItem('dh-lite-input-device', JSON.stringify({ id: 'IPH', label: "Jason's iPhone Microphone (Continuity)" }));
    const explicit = await recResolveAudioConstraints();
    localStorage.removeItem('dh-lite-input-device');
    navigator.mediaDevices.enumerateDevices = origEnum;
    return { noSaved, explicit };
  });
  ok(e2.noSaved.deviceId && e2.noSaved.deviceId.ideal === 'MAC1' && e2.noSaved.echoCancellation === false,
    'E2a browser default = continuity mic → steers to first non-continuity input (raw trio kept)');
  ok(e2.explicit.deviceId && e2.explicit.deviceId.ideal === 'IPH',
    'E2b explicitly saved continuity mic is honored (only auto-pick is blocked)');

  // ── E3: acquired device ≠ saved → retry with exact and swap streams ──
  const e3 = await page.evaluate(async () => {
    localStorage.setItem('dh-lite-input-device', JSON.stringify({ id: 'WANT', label: 'Want Mic' }));
    const mk = (id) => { const st = []; return {
      _stopped: st,
      getAudioTracks() { return [{ getSettings: () => ({ deviceId: id }) }]; },
      getTracks() { return [{ stop() { st.push(1); } }]; },
    }; };
    const wrong = mk('OTHER'), right = mk('WANT');
    const orig = navigator.mediaDevices.getUserMedia;
    let exactReq = null;
    navigator.mediaDevices.getUserMedia = async (c) => { exactReq = c.audio.deviceId && c.audio.deviceId.exact; return right; };
    const out = await recEnsureSavedInput(wrong);
    navigator.mediaDevices.getUserMedia = orig;
    localStorage.removeItem('dh-lite-input-device');
    return { swapped: out === right, exactReq, wrongStopped: wrong._stopped.length > 0, rightKept: right._stopped.length === 0 };
  });
  ok(e3.swapped && e3.exactReq === 'WANT' && e3.wrongStopped && e3.rightKept,
    'E3 wrong-device acquire retries with exact, returns the right stream, stops the wrong one');
