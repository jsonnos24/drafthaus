// lite-1.056: no-account chord tools on the landing (Quick Chords / Find a Chord),
// sign-in nudge, and Back routing to the landing for preview visitors.
const { chromium } = require('playwright-core');
const path = require('path'); const http = require('http'); const fs = require('fs');
const BUILD = 'lite-1.056.html';
...
(async () => {
  ...
  // ── Quick Chords from the landing (no auth) ──
  {
    ...
    assert('qc: key selector defaults to C / Major', await page.evaluate(() =>
      document.getElementById('qcKeyRoot').value === 'C major' && document.getElementById('qcKeyMode').value === 'major'));
  ...
  // 390×640 approximates an iPhone with the URL bar + bottom toolbar showing.
  {
    ...
    await page.screenshot({ path: '_shot_lite_1056_fit640.png' });
  }
  ...
  // ── Guest banner beside Songs + "Sign in to save" re-opens the login overlay ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1056_relogin.png' });
  ...
  // ── Guest banner screenshot ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1056_guestlist.png' });
  }
  ...
  // ── Quick Chords: two-dropdown key selector (root + major/minor) ──
  {
    const page = await (await desktopCtx()).newPage();
    ...
    await page.waitForTimeout(80);
    assert('qckey: the old key pill is gone, two <select>s present', await page.evaluate(() =>
      !document.getElementById('qcKeyPill') && !!document.getElementById('qcKeyRoot') && !!document.getElementById('qcKeyMode')));
    assert('qckey: defaults to C / Major; diatonic I chord is C', await page.evaluate(() =>
      document.getElementById('qcKeyRoot').value === 'C major' && document.getElementById('qcKeyMode').value === 'major' &&
      document.querySelector('#qcPills .qc-col .qc-pill').textContent.trim() === 'C'));
    assert('qckey: root dropdown lists 12 roots, chromatically ordered', await page.evaluate(() => {
      const o = [...document.getElementById('qcKeyRoot').options];
      return o.length === 12 && o[0].textContent === 'C' && o[1].textContent === 'D♭';
    }));
    // Switch to Minor → keeps the same root pitch (C), I chord becomes Cm, roots respell.
    await page.selectOption('#qcKeyMode', 'minor');
    await page.waitForTimeout(50);
    assert('qckey: Minor keeps the root (C minor), I chord becomes Cm', await page.evaluate(() =>
      document.getElementById('qcKeyRoot').value === 'C minor' &&
      document.querySelector('#qcPills .qc-col .qc-pill').textContent.trim() === 'Cm'));
    // Change root → key + diatonic update.
    await page.selectOption('#qcKeyRoot', 'G minor');
    await page.waitForTimeout(50);
    assert('qckey: changing root to G minor updates the diatonic I to Gm', await page.evaluate(() =>
      document.querySelector('#qcPills .qc-col .qc-pill').textContent.trim() === 'Gm'));
    assert('qckey: works with NO song loaded (guest, standalone)', await page.evaluate(() => _currentSong === null));
  }

  // ── Cm / C#m now show the A-shape barre voicing ──
  {
    const page = await (await desktopCtx()).newPage();
    page.on('pageerror', e => errors.push('PAGEERROR(barre): ' + e.message));
    await page.goto(base, { waitUntil: 'load' });
    const v = await page.evaluate(() => {
      const cm = fbGetVoicing('guitar', 'Cm'), csm = fbGetVoicing('guitar', 'C#m');
      return { cm: cm.strings.map(s => s.fret), cmB: cm.barres, csm: csm.strings.map(s => s.fret), csmB: csm.barres };
    });
    assert('barre: Cm = x35543 A-shape barre at fret 3', JSON.stringify(v.cm) === JSON.stringify([3, 4, 5, 5, 3, null]) && v.cmB.includes(3));
    assert('barre: C#m = x46654 A-shape barre at fret 4', JSON.stringify(v.csm) === JSON.stringify([4, 5, 6, 6, 4, null]) && v.csmB.includes(4));
    // Sanity: the override didn't break a normal open chord.
    assert('barre: C major still open (x32010-ish), no barre', await page.evaluate(() => {
      const c = fbGetVoicing('guitar', 'C');
      return JSON.stringify(c.strings.map(s => s.fret)) === JSON.stringify([0, 1, 0, 2, 3, null]) && c.barres.length === 0;
    }));
  }
  ...
  // ── Phase 1: uke barre bar, auto-play I chord, QC string names, borrowed layout ──
  {
    ...
    assert('autoplay: switching to A minor selects + would play Am', await page.evaluate(() => {
      let played = null; const orig = playChordOnInstrument; playChordOnInstrument = (n) => { played = n; };
      qcOnMode('minor'); qcOnRoot({ value: 'A minor' });
    ...
    assert('borrowed: each borrowed chord is a label-over-pill .qc-bcol', await page.evaluate(() => {
      ...
      return !!ivl && !!pill && !!(ivl.compareDocumentPosition(pill) & Node.DOCUMENT_POSITION_FOLLOWING);
    }));
  }

  // ── Phase 2: tuning layout + E♭ relabel + audio shift + lyrics-popover unaffected ──
  {
    const page = await (await desktopCtx()).newPage();
    ...
    await page.evaluate(() => openQuickChords());
    await page.waitForTimeout(60);
    assert('layout: root/mode group is left of the tuning dropdown', await page.evaluate(() => {
    ...
    await page.evaluate(() => { qcOnMode('minor'); qcOnRoot({ value: 'A minor' }); });
    await page.waitForTimeout(40);
    assert('relabel: A minor I reads "Am" in Standard', await page.evaluate(() =>
      document.querySelector('#qcPills .qc-col .qc-pill').textContent.trim() === 'Am'));
    ...
    assert('labels: E♭ string labels are flat-spelled (E♭/A♭, not D#/A#)', await page.evaluate(() => {
      const t = document.getElementById('qcNeck').textContent;
      return t.includes('E♭') && t.includes('A♭') && !t.includes('D#') && !t.includes('A#');
    }));
    assert('shapes: fret numbers identical between tunings', await page.evaluate(() => {
      ...
      _tuning = 'eb'; return JSON.stringify(a) === JSON.stringify(b);
    }));
    assert('audio: QC play drops a semitone in E♭', await page.evaluate(() => {
      let got = null; const orig = playNotesStrum; playNotesStrum = (m) => { got = m; };
      _tuning = 'standard'; playChordOnInstrument('Am', 'guitar', _tuneShift()); const std = got.slice().sort((a,b)=>a-b);
      _tuning = 'eb'; playChordOnInstrument('Am', 'guitar', _tuneShift()); const eb = got.slice().sort((a,b)=>a-b);
      playNotesStrum = orig;
      return eb.length === std.length && eb.every((m, i) => m === std[i] - 1);
    }));
    assert('audio: lyrics chord popover is NOT shifted in E♭ (cpPlay stays concert pitch)', await page.evaluate(() => {
      let got = null; const orig = playNotesStrum; playNotesStrum = (m) => { got = m; };
      _tuning = 'eb'; _cpName = 'Am'; _cpInst = 'guitar'; cpPlay(); const popMidis = got.slice().sort((a,b)=>a-b);
      _tuning = 'standard'; playChordOnInstrument('Am', 'guitar', 0); const stdMidis = got.slice().sort((a,b)=>a-b);
      playNotesStrum = orig;
      return popMidis.length === stdMidis.length && popMidis.every((m,i)=>m===stdMidis[i]); // popover == concert pitch
    }));
  ...
  // ── Phase 3: FC scrollable neck, open/mute cycle, tap-to-hear ──
  {
    ...
    await page.waitForTimeout(120);
    assert('fcneck: frets render up to 22 (guitar) in a scroll container', await page.evaluate(() => {
      const scroll = document.querySelector('#fcNeck div[style*="overflow-x"]');
      const html = document.getElementById('fcNeck').innerHTML;
      return !!scroll && html.includes('>22</text>') && !html.includes('>23</text>');
    }));
    ...
    assert('mute: muted strings excluded from fcPlacedMidis', await page.evaluate(() => {
      ...
      return fcPlacedMidis().length === 2; // open(0) + fret(3); mute excluded
    }));
    ...
    assert('fret: tapping a fret then again clears it', await page.evaluate(() => {
      fcInitPlaced(); fcTapFret(1, 7); const set = _fcPlaced[1]; fcTapFret(1, 7); const cleared = _fcPlaced[1];
      return set === 7 && cleared === null;
    }));
    assert('uke: neck renders up to 15 frets', await page.evaluate(() => {
      fcSetInstrument('ukulele'); const html = document.getElementById('fcNeck').innerHTML;
      fcSetInstrument('guitar'); return html.includes('>15</text>') && !html.includes('>16</text>');
    }));
    assert('cleanup: _fcBase no longer referenced (no crash on clear)', await page.evaluate(() => {
      try { fcClear(); return true; } catch (e) { return false; }
    }));
  }
  ...
  // ── Screenshots ──
  {
    const page = await (await mobileCtx()).newPage();
    await page.goto(base, { waitUntil: 'load' });
    await page.screenshot({ path: '_shot_lite_1056_landing.png' });
    await page.click('.landing-tools .ltool-btn:nth-child(1)'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1056_qc.png' });
    await page.click('#screen-qc .nav-switch'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1056_fc.png' });
  }
