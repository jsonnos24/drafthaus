// lite-1.043: Chordify centered between Play and 🎸; Play button green + bigger.
const { chromium } = require('playwright-core');
const path = require('path'); const http = require('http'); const fs = require('fs');
const BUILD = 'lite-1.043.html';
...
(async () => {
  ...
  // ── Play button green + bigger; Chordify centered between Play and 🎸 ──
  {
    ...
    assert('center: Chordify is vertically centered between Play and the 🎸 tool', await page.evaluate(() => {
      const play = document.getElementById('playBtn').getBoundingClientRect();
      const ch = document.getElementById('chordsModeBtn').getBoundingClientRect();
      const g = document.querySelector('.rail .rail-tool').getBoundingClientRect();
      const above = ch.top - play.bottom, below = g.top - ch.bottom;
      return above > 10 && below > 10 && Math.abs(above - below) < 10;
    }));
  }
