// lite-1.050: no-account chord tools on the landing (Quick Chords / Find a Chord),
// sign-in nudge, and Back routing to the landing for preview visitors.
const { chromium } = require('playwright-core');
const path = require('path'); const http = require('http'); const fs = require('fs');
const BUILD = 'lite-1.050.html';
...
(async () => {
  ...
  // 390×640 approximates an iPhone with the URL bar + bottom toolbar showing.
  {
    ...
    await page.screenshot({ path: '_shot_lite_1050_fit640.png' });
  }
  ...
  // ── Guest banner beside Songs + "Sign in to save" re-opens the login overlay ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1050_relogin.png' });
  ...
  // ── Guest banner screenshot ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1050_guestlist.png' });
  }
  ...
  // ── Export internals: token parsing, PDF output, zip structure (stubbed takes) ──
  {
    ...
    if (libsLoaded) {
      ...
      assert('name: unnamed take → "Take 1 - <creation date/time>.wav"', keys.files.some(k => /My Song\/Takes\/Take 1 - June 6th, 2026 - 3-42pm\.wav/.test(k)));
      assert('name: renamed take → "<name> - Take 2 - <creation date/time>.wav"', keys.files.some(k => /My Song\/Takes\/My Take - Take 2 - June 7th, 2026 - 9-05am\.wav/.test(k)));
    }
  }
  ...
  // ── Screenshots ──
  {
    const page = await (await mobileCtx()).newPage();
    await page.goto(base, { waitUntil: 'load' });
    await page.screenshot({ path: '_shot_lite_1050_landing.png' });
    await page.click('.landing-tools .ltool-btn:nth-child(1)'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1050_qc.png' });
    await page.click('#screen-qc .nav-switch'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1050_fc.png' });
  }
