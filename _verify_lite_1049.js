// lite-1.049: no-account chord tools on the landing (Quick Chords / Find a Chord),
// sign-in nudge, and Back routing to the landing for preview visitors.
const { chromium } = require('playwright-core');
const path = require('path'); const http = require('http'); const fs = require('fs');
const BUILD = 'lite-1.049.html';
...
(async () => {
  ...
  // 390×640 approximates an iPhone with the URL bar + bottom toolbar showing.
  {
    ...
    await page.screenshot({ path: '_shot_lite_1049_fit640.png' });
  }
  ...
  // ── Guest banner beside Songs + "Sign in to save" re-opens the login overlay ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1049_relogin.png' });
  ...
  // ── Guest banner screenshot ──
  {
    ...
    await page.screenshot({ path: '_shot_lite_1049_guestlist.png' });
  }
  ...
  // ── Export button: placement + desktop-only gating ──
  {
    ...
    assert('export: no songs → toast "No songs to export", stays idle', await page.evaluate(() => {
      if (_songsUnsub) { _songsUnsub(); _songsUnsub = null; } _songs = [];
      exportAll();
      return /No songs to export/.test(document.getElementById('toast').textContent) && _exporting === false;
    }));
  }
  // Desktop-only: hidden on mobile.
  {
    ...
    await page.click('.auth-card .auth-btn.ghost');
    await page.waitForSelector('body.signed-in', { timeout: 15000 });
    assert('export: button hidden on mobile (computed display none)', await page.evaluate(() =>
  ...
  // ── Export internals: token parsing, PDF output, zip structure (stubbed takes) ──
  {
    ...
    assert('libs: JSZip + jsPDF available after lazy-load', !libsLoaded ? false : await page.evaluate(() =>
      !!window.JSZip && !!(window.jspdf && window.jspdf.jsPDF)));
    ...
    if (libsLoaded) {
      ...
      const keys = await page.evaluate(async () => {
        // Stub: one take per song, fake audio + decode so no real network/Storage is hit.
        const realColl = db.collection.bind(db);
        db.collection = (n) => n === 'voice_takes'
          ? { where: () => ({ get: () => Promise.resolve({ forEach: cb => cb({ id: 't1', data: () => ({ songId: 'X', name: 'My Take', downloadUrl: 'blob:fake', createdAt: { toMillis: () => 1 } }) }) }) }) }
          : realColl(n);
        window.fetch = () => Promise.resolve({ arrayBuffer: () => Promise.resolve(new ArrayBuffer(8)) });
        const ctx = ensureCtx(); ctx.decodeAudioData = () => Promise.resolve(ctx.createBuffer(1, 200, 44100));
        const songs = [
        ...
        const { zip, skipped } = await _buildExportZip(songs);
        return { files: Object.keys(zip.files), skipped };
      });
      ...
      assert('zip: takes land in SONGNAME/Takes/<name>.wav', has('My Song/Takes/My Take.wav'));
      assert('zip: duplicate song titles are deduped (My Song (2))', keys.files.some(k => k.indexOf('My Song (2)/') === 0));
      assert('zip: no takes skipped with a valid stubbed take', keys.skipped === 0);
    }
  }
  ...
  // ── Screenshots ──
  {
    const page = await (await mobileCtx()).newPage();
    await page.goto(base, { waitUntil: 'load' });
    await page.screenshot({ path: '_shot_lite_1049_landing.png' });
    await page.click('.landing-tools .ltool-btn:nth-child(1)'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1049_qc.png' });
    await page.click('#screen-qc .nav-switch'); await page.waitForTimeout(120);
    await page.screenshot({ path: '_shot_lite_1049_fc.png' });
  }
