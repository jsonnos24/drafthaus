...
const p=await(await b.newContext({viewport:{width:390,height:540},hasTouch:true,isMobile:true,deviceScaleFactor:2})).newPage();
await p.goto(`http://127.0.0.1:${port}/lite-1.041.html`,{waitUntil:'load'});
await p.click('.auth-btn.ghost'); await p.waitForSelector('body.signed-in',{timeout:15000});
await p.evaluate(()=>{_openSongObj({id:'S',title:'My Song',key:'C major',lyricsDoc:'<div>I walked the <span class="chord" contenteditable="false">G</span>line for you</div>'});stopTakesListener();});
await p.waitForTimeout(150);
await p.screenshot({path:'_shot_songhdr2.png'});
