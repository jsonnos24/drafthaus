// lite-1.039: "+ Chords" toggle moved into the song header; Bold + lyr-toolbar removed.
const { chromium } = require('playwright-core');
const path = require('path'); const http = require('http'); const fs = require('fs');
const BUILD = 'lite-1.039.html';
...
(async () => {
  ...
  // ── "+ Chords" in the header; Bold + toolbar removed; tap-a-word still works ──
  {
    ...
    await page.goto(base, { waitUntil: 'load' });
    await openGuestSong(page, '<div>I walked the line</div>');
    assert('hdr: "+ Chords" button lives in the song header next to the theme icon', await page.evaluate(() => {
      const btn = document.querySelector('#screen-song .song-nav #chordsModeBtn');
      return !!btn && btn.textContent.trim() === '+ Chords' && !!document.querySelector('#screen-song .song-nav #themeToggleBtn');
    }));
    ...
    await page.evaluate(() => toggleChordsMode());
    assert('hdr: toggling off → "+ Chords" + editor editable', await page.evaluate(() => {
      const btn = document.getElementById('chordsModeBtn');
      return btn.textContent.trim() === '+ Chords' && document.getElementById('lyricsEditor').getAttribute('contenteditable') === 'true';
    }));
  }
