...
await p.goto(`http://127.0.0.1:${port}/lite-1.043.html`,{waitUntil:'load'});
...
await p.evaluate(()=>{_openSongObj({id:'S',title:'My Song',key:'C major',lyricsDoc:'<div>I walked the <span class="chord" contenteditable="false">G</span>line for you</div>'});stopTakesListener();
  // enable play to show the green colour (as if a take is loaded)
  document.getElementById('playBtn').classList.remove('disabled');});
await p.waitForTimeout(150);
await p.screenshot({path:'_shot_rail3.png'});
