// Lite 1.086: mobile playback-session robustness (bg audio, wake lock, sketchpad z-index, mic tip).
...
(async () => {
  ...
  await page.goto(`http://127.0.0.1:${port}/lite-1.086.html`, { waitUntil: 'load' });
  ...
  // ── boost removed (lite-1.086) ──
  assert('no #boostToggle element', await page.evaluate(() => !document.getElementById('boostToggle')));
  assert('no .boost-row element', await page.evaluate(() => !document.querySelector('.boost-row')));
  assert('toggleBoost() removed', await page.evaluate(() => typeof toggleBoost === 'undefined'));
  assert('updateBoostUI() removed', await page.evaluate(() => typeof updateBoostUI === 'undefined'));
  assert('_applyPlayVolume() removed', await page.evaluate(() => typeof _applyPlayVolume === 'undefined'));
  await page.evaluate(() => _openSongObj({ id: 'S2', title: 'Boost Test', key: 'C major', lyricsDoc: '<div>x</div>' }));
  assert('playback still works after boost removal (playEl gets src + play())', await page.evaluate(async () => {
    window.__pl2 = null;
    window._getPlayableSrc = async () => 'about:blank#t2';
    const p = HTMLMediaElement.prototype.play; HTMLMediaElement.prototype.play = function(){ window.__pl2 = this.id; return Promise.resolve(); };
    _takes = [{ id: 'T1', name: 'Take 1', downloadUrl: 'about:blank', mimeType: 'audio/webm', createdAt: 1 }];
    _loadedTakeId = 'T1';
    await playTake('T1');
    HTMLMediaElement.prototype.play = p;
    const el = document.getElementById('playEl');
    return el.src.includes('about:blank#t2') && window.__pl2 === 'playEl' && _playingTakeId === 'T1' && el.volume === 1;
  }));
