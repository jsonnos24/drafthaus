...
const p=await(await b.newContext({viewport:{width:390,height:740},hasTouch:true,isMobile:true,deviceScaleFactor:3})).newPage();
await p.goto(`http://127.0.0.1:${port}/lite-1.044.html`,{waitUntil:'load'});
await p.click('.auth-btn.ghost'); await p.waitForSelector('body.signed-in',{timeout:15000});
await p.evaluate(()=>{_openSongObj({id:'S',title:'My Song',key:'C major',lyricsDoc:'<div>x</div>'});stopTakesListener();document.getElementById('playBtn').classList.remove('disabled');});
await p.waitForTimeout(120);
const box = await (await p.$('.rail')).boundingBox();
await p.screenshot({path:'_shot_rail5.png', clip:{x:box.x-2, y:0, width:box.width+4, height:175}});
